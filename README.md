...
- 项目：搜索筛选、新建编辑、状态维护、负责人、周期、安全归档、恢复与受控永久删除
...
若本地演示账号已经修改，可通过 `NEXT_PUBLIC_DEMO_EMAIL` 与 `NEXT_PUBLIC_DEMO_PASSWORD` 同步登录页自动填充值；这两个变量只应在本地演示环境使用。
...
| PATCH / DELETE | `/api/projects/:id` | 更新或恢复项目 / 归档项目；超级管理员可永久删除已归档项目 |
...
所有业务接口都会在服务端验证登录会话和请求来源。普通用户只能访问已加入的项目；项目负责人或项目 manager 维护项目和迭代，研发成员维护自己开发或创建的任务，测试人员只能更新自己负责验收任务的状态并登记对应测试工时，只读用户不能写入任务或工时。测试人员不能担任项目负责人或开发负责人。指派测试负责人的任务必须先进入“待评审”，再由测试负责人或项目管理者验收完成；任务实际工时只能由工时明细自动汇总，不能在任务表单中手工修改。

任务至少登记一条实际工时后才能完成，已完成任务不能删除最后一条工时。历史数据中若存在“已完成但实际工时为 0”的任务，项目管理者可从任务卡点击“补录实际工时”；该受控入口只修复缺失工时，补录后已完成迭代继续保持只读。

同一项目只允许一个进行中迭代，已完成迭代会锁定为历史快照。项目归档后任务、迭代和工时均只读；已完成任务保留原开发与测试负责人，归档项目和停用成员的历史工时仍会进入周期报表和筛选项。项目负责人、项目 manager 或超级管理员可以恢复归档项目；只有超级管理员输入项目代号并再次确认后，才能永久删除归档项目及其全部业务数据。
...
- 应用进程会复用单个 PostgreSQL 客户端（每实例最多 1 个连接），避免生产请求重复创建连接；Neon 冷启动时可通过 `DATABASE_CONNECT_TIMEOUT_SECONDS` 调整连接等待时间。
